...
lib.mkIf config.purpose.daily {
  home.packages =
    let
      mkLedger =
        ...
        pkgs.writeShellApplication {
          ...
          text = ''
            LEDGER_FILE="${config.home.homeDirectory}/git/ledgers/${subpath}/main.journal"
            export LEDGER_FILE
    ...
    in
    [
      (mkLedger "hledger" "personal")
      (mkLedger "hledger-lab" "lab")
    ];

  home.file.".hledger/hledger.conf".text = ''
    --encoding=utf-8
    --auto
